...
Examples:
    ...
    python cam4_standalone.py --flaresolverr http://localhost:8191 https://www.cam4.com/performer
...
class FlareSolverrClient:
    ...
    FlareSolverr must be running as a service (typically on port 8191).
    ...
    def _request(self, cmd: str, url: str = None, **kwargs) -> dict:
    ...
    def get(self, url: str, timeout: int = 10) -> 'FlareSolverrResponse':
        ...
        Returns a response-like object with status_code, text, and json() method.
        """
        result = self._request("request.get", url, maxTimeout=timeout * 1000)
...
class CAM4Standalone:
    ...
    def __init__(
        ...
        # Initialize FlareSolverr client if URL provided
        if flaresolverr_url:
            self._flaresolverr = FlareSolverrClient(flaresolverr_url)
            if self._flaresolverr.is_available():
                self._log(f"FlareSolverr available at {flaresolverr_url}")
            else:
        ...
        if USING_CURL_CFFI and not use_flaresolverr:
            ...
            if use_flaresolverr and self._flaresolverr:
                self._log("Using FlareSolverr for requests")
            else:
    ...
    def _make_request(self, url: str, timeout: int = 10):
        ...
        1. FlareSolverr (if enabled and available)
        2. curl_cffi / requests session
        """
        # Try FlareSolverr first if enabled
        if self._use_flaresolverr and self._flaresolverr:
            try:
                self._log(f"Requesting via FlareSolverr: {url}")
                return self._flaresolverr.get(url, timeout=timeout)
            except Exception as e:
                self._log(f"FlareSolverr failed: {e}, falling back to session")
    ...
    def verify_stream_accessible(self, m3u8_url: str) -> Tuple[bool, Optional[str]]:
        ...
        try:
            response = self._make_request(m3u8_url, timeout=10)
...
Examples:
    ...
    Use FlareSolverr (for bypassing anti-bot protections):
        python cam4_standalone.py --flaresolverr http://localhost:8191 https://www.cam4.com/performer
        """
    )
    ...
    parser.add_argument('--flaresolverr', metavar='URL', help='FlareSolverr URL (e.g. http://localhost:8191)')
    ...
    cam4 = CAM4Standalone(
        ...
        use_flaresolverr=bool(args.flaresolverr)
    )
